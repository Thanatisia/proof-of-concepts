...
declare -A vnc_server_Settings=(
    ## VNC server settings
    ["host"]="127.0.0.1"
    ["port"]=5906
    ["display"]=":6"
    ["flags"]="-nopw -xkb -forever -bg"
)
declare -A websocket_Settings=(
    ## Websocket server settings
    ["port"]="6081"
    ["client"]="$HOME/Desktop/repos/git/novnc"
    ["flags"]="-D"
)
declare -A BUFFERS=(
    [0]="0;1024x768x16;"
...
main()
{
    ...
    x11vnc -display ${vnc_server_Settings["display"]} -rfbport ${vnc_server_Settings["port"]} ${vnc_server_Settings["flags"]}
    ...
    websockify ${websocket_Settings["flags"]} --web=${websocket_Settings["client"]} ${websocket_Settings["port"]} ${vnc_server_Settings["host"]}:${vnc_server_Settings["port"]}
